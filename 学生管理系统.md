...
- **后端技术栈**: Go (Golang) 1.21+ + Gin (Web框架) + Gorm (ORM)
    
- **前端技术栈**: Vue.js (v3) + Vue Router + Pinia + Axios + Element Plus + Vite
...
### 2.1. 系统管理与权限 (对应要求 6) ✅ 已实现

- **用户管理**: ✅ 管理员可以添加、修改、删除系统用户（如教师、学生账户），用户状态管理，角色分配。
...
- **角色与权限**: ✅ 已实现
    ...
    - ✅ 细粒度权限控制（学生、班级、课程、成绩、考勤等模块的读写权限）。

    - ✅ 动态权限配置：管理员可在前端界面配置角色权限。
...
- **统计分析**: ✅ 已实现
    ...
    - ✅ 管理员统计概览（用户、学生、课程、成绩、考勤等统计数据）。
        

### 2.2. 学生信息管理 (对应要求 1) ✅ 已实现
...
### 2.3. 课程与成绩管理 (对应要求 2) ✅ 已实现

- **课程管理**: ✅ 已实现
    
    - ✅ 管理员或教师（需授权）可以 CRUD 课程信息（课程名称、授课教师、学分等）。
        
    - ✅ 课程列表展示（分页、搜索：课程名称、授课教师）。
...
- **成绩录入**: ✅ 已实现
    ...
    - ✅ 教师可以为所教课程的学生录入考试成绩和（或）平时表现成绩。
        
    - ✅ 成绩录入时自动创建选课关系（如果不存在）。
        
- **成绩查询**: ✅ 已实现
    
    - ✅ 学生/家长可以查询自己的成绩（按学生ID查询）。
        
    - ✅ 教师可以查询所教课程的成绩（按课程ID查询）。
        
    - ✅ 管理员可以查询所有成绩（成绩列表，分页、筛选）。
        

### 2.4. 学生考勤管理 (对应要求 3) ✅ 已实现

- **考勤记录**: ✅ 已实现
    
    - ✅ 教师（或班主任）可以记录学生的出勤情况（签到、缺席、请假、迟到）。
...
- **考勤统计**: ✅ 已实现
    ...
    - ✅ 按班级、按时间范围统计（已实现基础统计页面）。
    

### 2.5. 学生奖惩管理 (对应要求 4) ✅ 已实现

- **奖惩记录**: ✅ 已实现
    
    - ✅ 教师或管理员可以录入学生的奖励和处分情况。
        
    - ✅ 奖惩记录CRUD操作。
        
- **记录类型**: ✅ 已实现
    
    - ✅ 区分奖励/处分，并记录事由、日期、级别等。
...
### 2.6. 家长联系管理 (对应要求 5) ✅ 已实现

- **家长信息**: ✅ 已实现
    
    - ✅ 在学生信息中关联家长的联系方式（独立的家长表）。
...
- **通知功能**: ✅ 已实现
    ...
    - ✅ 模拟发送通知、短信、邮件的功能接口（注：实际发送需对接第三方服务，项目初期只做记录）。
